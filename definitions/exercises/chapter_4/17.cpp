...
int main(int argc, char **argv[]){
  ...
  time_t end = time(0);
  cout << "The time to add 10000000 items to a queue with the linked list implementation " << difftime(end,begin) << endl;
  
  begin = time(0);
  for(i=0;i<size;i++){
    dline.enqueue_front(i);
  }
  end = time(0);
  cout << "The time to add 10000000 items to a dequeue with the linked list implementation " << difftime(end,begin) << endl;
  begin = time(0);
  for(i=0;i<size;i++){
    vec[i] = i;
  }
  end = time(0);
  cout << "The time to add 10000000 items to a vector " << difftime(end,begin) << endl;
