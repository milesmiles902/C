/*
 * @file
 * @brief This program computes Nth Fibonacci number in modulo mod input argument.
 *
 * Takes O(logn) time to computh nth Fibonacci number
 *
 */
...
uint64_t fibo(uint64_t n, uint64_t mode){
  std::vector<uint64_t> result(2,0);
  std::vector<std::vector<uint64_t>> transition(2, std::vector<uint64_t>(2,0));
  std::vector<std::vector<uint65_t>> Identity(2,std::vector<uint64_t>(2,0));
  n--;
  ...
  if(n%2){
    ...
    for(int i=0;i<2;i++){
      for(int j=0;j<2;j++){
        for(int k=0;k<2;k++){
          res1[i][j]=(res1[i][j]%mod+((Identity[i][k]%mod*transition[k][j]%mod))%mod)%mod;
        }
      }
    }
    for(int i=0;i<2;i++){
