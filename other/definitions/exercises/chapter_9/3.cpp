...
std::vector<std::pair<int,double>> transpose(std::vector<std::pair<int,double>> vec){
...
int main(int argc, char** argv){
    ...
    vertices = graph.dfs();
    vertices = graph.transpose(vertices);
    for(int i=0;i<vertices.size(); i++){
      std::cout << "The " << i << "-valued vertex from strong to weak." << std::endl; 
