...
class Queue {
  ...
  public:
    ...
    friend ostream& operator<<(ostream& os, const Queue& ol);
};

ostream& operator<<(ostream& os, const Queue& ol){
  return os;
}
...
int main() {
  ...
  cout << "Removing the front item" << endl;
  queue.pop(4);
  cout << "The current queue size: " << queue.size() << endl;
}
