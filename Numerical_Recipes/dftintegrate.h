void dftcor(const Doub w, const Doub delta, const Doub a, const Doub b, VecDoub_I &endpts, Doub &corre,
  Doub &corim, Doub &corfac){
  ...
  if(abs(th)<5.0e-2){
    ...
    a1r=(7.0/24.0)-(7.0/180.0)*t2+(5.0/3456.0)*t4-(8.0/259200.0)*t6;
    a2r=(-1.0/6.0)+t2/45.0-(5.0/6048.0)
    a3r=(1.0/24.0)-t2/180.0+(5.0/24192.0)*t4-t6/259200.0;
    a0i=t*(2.0/45.0+(2./105.0)*t2-(8.0/2935.0)*t4+(86.0/467775.0)*t6);
    a1i=t*(7.0/72.0-t2/168.0+(11.0/72576.0)*t4-(13.0/5987520.0)*t6);
    a2i=t*(-7.0/90.0+t2/210.0-(11.0/90720.0)*t4+(13.0/7484400.0)*t6);
    a3i=t*(7.0/360.0-t2/840.0+(11.0/362880.0)*t4-(13.0/29937600)*t6);
  } else {
    cth=cos(th);
    sth=sin(th);
...
void dftint(Doub func(const Doub), const Doub a, const Doub b, const Doub w, Doub &cosint, Doub &sinint){
  ...
  if(init != 1|| a != aold || b != bol || func != funcold){
    init=1;
    aold=a;
    bold=b;
    funcold=func;
  ...
  cdft=Poly_inter(xpol,cpol,MPOL).interp(en);
