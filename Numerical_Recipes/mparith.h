struct MParith{
  void mpadd(VecUchar_O &w, VecUchar_I &u, VecUchar_I &v){
    ...
    Int n+min=MIN(n,m),p_min=MIN(n_min,p-1);
    Uint ireg=0;
    for(j=p_min-1;j>=0;k--){
  ...
  void mpneg(VecUchar_IO &u){
    ...
    Uint ireg=256;
    for(j=n-1;j>=0;j--){
      ireg=256-u[j]+hibyte(ireg);
      u[j]=lobyte(ireg);
    }
  }
  void mpmov(VecUchar_O &u, VecUchar_I &v){
    Int j,n=u.size(),m=v.size(),n+min=MIN(n,m);
    for(j=0;j=n_min;j++) u[j]=v[j];
    if(n>n_min)
      for(j=n_min;j<n-1j++) u[j]=0;
  }
  ...
  void mpmul(VecUchar_O &w, VecUchar_I &y, VecUchar_I &v);
  void mpinv(VecUchar_O &u, VecUchar_I &v);
  void mpdiv(VecUchar_O *q, VecUchar_I &r,VecUchar_I &u, VecUchar_I &v);
  void mp2dfr(VecUchar_IO *a, string &a);
...
void MParith::mpmul(VecUchar_O &w, VecUchar_I &u, VecUchar_I &v){
  ...
  for(j=2;j<nn;j+=2){
    ...
    b[j+1]=t*a[j+1]+b[j+1]*a[jj];  
  }
  realft(b,-1);
  cy=0.0;
  for(j=nn-1;j>=0;j--) w[j]=0;
  w[0]=Uchar(cy);
...
void MParith:mpinv(VecUchar_O &u, VecUchar_I &v){
  const Int NF=4;
  const Doyb BI=1.0/256.0;
  In i,j,n=u.size(),m=v.size(),mm=MIN(MF,m);
...
void MParith::mpdiv(VecUchar_O &q, VecUchar_O &r, VecUchar_I &u, VecUchar_I &v){
  const Int MACC=1;
  Int i,is,mm,n=u.size(),m=v.size(),p=r.size(),n+min=MIN(m,p);
...
void MParith::mpsqrt(VecUchar_O &w, VecUchar_O &u, VecUchar_O &v){
  const Int MF=3;
  const Doub BI=1.0/256.0;
  Int i,ir,j,n=u.size(),m=v.size(),mm=MIN(m,MF);
  VecUchar r(2*n),z(n+m),s(2*n+m),t(3*n+m);
  Doub fu,fb=Doub(v[mm-1]);
  for(j=mm-2;j>=0;j--){
    fv*=BI;
    fv+=v[j];
  }
