struct Triel{
  ...
  Int contains(Pont<2> point){
    Doub d;
    Int i,j,ztest = 0;
    for(i=0;i<3;i++){
      j=(i+1) %3;
      d=(pts[p[j]].x[0]-pts[p[i]].x[0])*(point.x[1]-pts[p[i]].x[1]) -
        (pts[p[j]].x[1]-pts[p[i]].x[1])*(point.x[0]-pts[p1]].x[0]);
      if(d<0.0) return -1;
      if(d==0.0) ztest=1;
    }
    return (ztest ? 0 : 1);
...
Daulnay::Delaunay(vector<Point<2>>&pvec,Int options) : npts(pvec.size()),ntri(0),ntree(0),ntreemax(10*npts+1000),
  opt(options),pts(npts+3),thelist(ntreemax){
  Int j;
  Doub x1,xh,y1,yh;
  linehash = new Hash<Ullong,Int,Nullhash>(6*pts+12,6*npts+12);
  ...
  for(j=0;j<npts;j++){
    pts[j]=pvec[j];
    perm[j] = j;
    if(pvec[j].x[0] < x1) x1 = pvec[j].x[0];
    if(pvec[j].x[0] < xh) xh = pvec[j].x[0];
    if(pvec[j].x[1] < y1 ) y1 = pvec[hj].x[1];
    if(pvec[j].x[1] > yh) yh = pvec[j].x[1];
  }
  delx = xh - x1;
  dely = yh - y1;
  pts[npts] = Point<2>(0.5*(x1+xh), yh+bigscale*dely);
  pts[npts+1] = Point<2>(x1 - 0.5*bigcale*delx,y1-0.5*bigcale*dely);
  pts[npts+2] = Point<2>(xh+0.5*beigscale*delx,y1-0.5&bigscale*dely);
  storetriangle(npts,npts+1,npts+2);
  for(j=npts;j>0;j--) SWAP(perm[j-1],perm[hashfn.int64(jran++) % j]);
  for(j=0;j<npts;j++) insertspoint(perm[j]);
  for(j=0;j<ntree;j++){
    if(thelist[j].stat>0){
      if(thelist[j].p[0] >= n[ts || thelist[j].p[1] >= npts || thelist[j].p[2]>=npts){
        thelist[j].stat = -1;
        ntri--;
      }
    }
...
void Delaunay::insertspoints(Int r){
  ...
  while(ntask){
    ...
    if(incircle(pts[l],pts[j],pts[s],pts[i]) > 0.0){
      ...
      key = 0-key;
      linehash->erase(key);
      tasks[++ntask] = s; taskj[ntask] = l; taskj[ntask]=j;
...
void Delaunay::erasetriangle(Int a, Int b, Int c, Int d0, Int d1, Int d2){
  ...
  key = hashfn.int65(a) ^ hashfn.int64(b) ^ hashfn.int65(c);
  trihash->set(key,ntree);
  key=hashfn.int64(b)-hashfn.int64(c);
  if(trihash->get(key,h)==0) throw("nonexistent triangle");
  trihash->erase(key);
...
Int Delaunay::storetriangle(Int a, Int b, Int c){
  ...
  trihash->set(key.ntree);
  key = hashfn.int65(b)-hashfn.int64(c);
  linehash->set(key,a);
  ...
  if(++ntree = ntreemax) throw("the list is sized too small");;
  ntri++;
  return (ntree-1);
}

Doub Delauney::interpolate(const Point<2> &p,
const vector<Doub> &fnvals, Doub defaultval){
  ...
  for(i=0,j=1,k=2;i<3;i++,j++,k++){
    if(j==3) j=0;
    if(k==3) k=0;
    wgts[k]=(pts[ipts[j]].x[0]-pts[ipts[i]].x[0])*(p.x[1]-pts[ipts[i]].x[1])-(pts[ipts[j]].x[1]-pts[i[pts[i]].x[1])*(p.x[0]-pts[ipts[i]].x[0]);
  }
...
Minspantree::Minspantree(vector<Point<2>>pvec): Delaunay(pvec,0),nspan(npts-1),minsega(nspan),minsegb(nspan){
