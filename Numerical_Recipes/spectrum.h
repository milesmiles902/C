...
struct Slepian : Spectreg {
  ...
  Slepian(Int em, Int jjres, Int kkt)
   : Spectreg(em), jres(jjres), kt(kkt), dpss(kkt,2*em){
    if(jres<1||kt>=2*jres) throw("kt too big for jres too small");
    filltable();
  }
  void filltable();
  void renorm(Int n){
...
void Slepian::filltable(){
  ...
  Doub xx,xnew,xcol,sw,ppp,ddd,sum,bet,ssub,ssup,*u;
  Int i,j,k,nl;
  VecDoub dg(m2),dgg(m2),gam(m2),sup(m2-1),sub(m2-1);
  sw=2.*SQR(sin(jres*PI/m2));
  dg[0]=0.25*(2*m2+sq*SQR*m2-1.)-1.);
  for(i=1;i<m2;i++){
  ...
  for(k=0;k<kt;k++){
    ...
    n1=m2/3;
    dgg[n1]=1.;
    ssup=sup[n1]; ssub=sub[n1-1];
    u[0]=sup[n1]=sub[n1-1]=0.;
    bet=dgg[0];
    for(i=1;i<m2;i++){
      gam[i]=sup[i-1]/bet;
      bet=dgg[i]-sub[i-1]*gam[i];
      u[i]=((i==n1?1.:0)-sub[i-1]*u[i-1])/bet;
    }
    for(i=m2-2;i>=0;i--) u[i]-=gam[i+1]*u[i+1];
    sup[n1]=ssup;sub[n1-1]=ssub;
    sum=0.;
