struct Chebyshev {
  ...
  Chebyshev(VecDoub &cc, Doub aa, Doub bb) : n(cc.size()), m(n), c(cc), a(aa), b(bb) {};
...
Chebyshev::Chebyshev(Doub func(Doub), Doub aa, Doub bb, Int nn=50) : n(nn), m(nn), c(n), a(aa), b(bb) {
...
Chebyshev::Chebyshev(VecDoub &d) : n(d.size()), m(n), c(n), a(-1.), b(1.){
  c[n-1]=d[n-1];
  c[n-2]=2.0*d[n-2];
  for(Int j=n-3;j>=0;j--){
    c[j]=2.0*d[j]+c[j+2];
