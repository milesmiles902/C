...
struct Hashfn2 {
  ...
  Ullong fn(const void *key)P
    Int j;
    char *k = (char *)key;
    h=0xBB40E64DA205B064LL;
    j=0;
    while(n ? j++ <n : *k){
      h=(h*766434582181590749LL)^hashfn_tab[(unsigned char)(*k)];
      k++;
    }
    return h;
  {
...
struct Hash : Hashtable<keyT, hfnT> {
  ...
  void set(const keyT &key, const eltT &el)
    {els[iset(key)]=el;}
...
struct Mhash : Hashtable<keyT,hfnT> {
  ...
  Int erase(const key T &key, const elT &el);
