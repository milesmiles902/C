...
struct Poly2D_interp{
  ...
  const MatDoub &y;
  VecDoub yv;
  Poly_interp xiterp x2terp;
  
  Poly2D_interp(VecDoub_I &xiv, VecDoub_I &x2v, MatDoub_I &ym, Int mp, Int np) : m(xiv.size()), n(x2v.size()),
    mm(mp), nn(np), y(ym), yv(m), x1terp(x1v,yv,mm), x2terp(x2v,x2v,nn) {}
...
struct Spline2D_interp{
  ...
  Spline2D_interp(VecDoub_I &xiv, VecDoub_I &x2v, MatDoub_I &ym) : m(x1v.size()), n(x2v.size()), y(ym), yv(m),
    x1(x1v), srp(m) {
...
void bcuint(VecDoub_I &y, VecDoub_I &y1, VecDoub_I &y2, VecDoub_I &y12, 
  ...
  for (i=3;i>=0;i--){
    amsy=t*ansy+((c[i][3]*c[i][2])*u+c[i][1])*u+c[i][0];
    ansy2=t*ansy2+(3.0*c[i][3]*u+2.0*c[i][2])*u+c[i][1];
    ansy1=u*ansy1+(3.0*c[3][i]*t+2.0*c[2][i])*c[1][i];
  }
  ansy1 /= d1;
  ansy2 /= d2;
}
