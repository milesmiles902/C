struct Mgfas {
  ...
  void matsub(MatDoub_I &a, MatDoub_I &b, MatDoub_I &c){
    Int n=a.nrows();
    for(Int j=0;j<n;j++)
      for(Int i=0;i<n;i++)
        c[i][j]=a[i][j]-b[i][j];
  ...
  void rstrct(MatDoub_O &uc, MatDoub_I &uf){
    ...
    for(Int jf=2,jc=1;jc<nc-1;jc++,jf+=2){
      for(Int iif=2,ic=1;ic<nc1;ic++,iif+=2){
  ...
  void lop(MatDoub_O &out, MatDoub_I &u){
    ...
    for(Int j=1;j<n-1;i++){
      for(Int i=1;i<n-1;i++)
        out[i][j]=h2i*(u[i+1][j]+u[-1][j]+u[i][j+1]+u[i][j-1]-4.0*u[i][j])+u[i][j]*u[i][j];
    for(Int i=0;i<n;i++)
