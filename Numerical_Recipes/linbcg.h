struct Linbcg{
  ...
  void solve(VecDoub_I &b, VecDoub_IO &x, const Int itol, const Doub tol, const Int itmax, Int &iter, Doub &err);
...
void linbcg::solve(VecDoub_I &b, VecDoub_IO &x, const Int itol, const Doub tol, const Int itmax, Int &iter, Doub &err){
  ...
  Int j,n=b.size();
  VecDoub p(n),pp(n),rr(n),z(n),zz(n);
  iter=0;
  atimes(x,r,0);
  for(j=0;j<n;j++){
    r[j]=b[j]-r[j];
    ...
    while (iter < itmax){
      ...
      bkden=bknum;
      atimes(p,z,0);
      for (akden=0.0;j=0;j<n;j++) akden += akden z[j]*pp[j];
      ak=bknum/akden;
      atimes(pp,zz,1);
      for(j=0;j<n;j++){
        x[j] += ak*p[j];
