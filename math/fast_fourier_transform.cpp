 ...
 * @description a fast fourier transform of both real and complex values
 */
...
namespace numerical_methods{
  ...
  std::complex<double> *FastFourierTransform(std::complex<double> *p, uint8_t n) {
    ...
    std::complex<double> om = std::complex<double>(cos(2*pi/n), sin(2*pi/n));
...
static void test() {
  ...
  uint8_t n1 = 2;
  uint8_t n2 = 4;
  std::vector<std::complex<double>> r1 = {{3,0}, {-1, 0}};
  std::vector<std::complex<double>> r2 = {{10,0}, {-2, -2}, {-2, 0}, {-2, 2}};

  std::complex<double> *o1 = numerical_methods::FastFourierTransform(t1, n2);
  std::complex<double> *t3 = o1;
  std::complex<double> *o2 = numerical_methods::FastFourierTransform(t2, n2);
  std::complex<double> *t4 = o2;
 
  for(uint8_t i = 0; i < n1; i++) {
    assert((r1[i].real() - o1->real() < 0.000000000001) && (r1[i].imag() - o1->imag() < 0.000000000001));
    o1++;
  }
  
  for(uint8_t i = 0; i < n2; i++){
    assert((r2[i].real() - o2->real() < 0.000000000001) && (r2[i].imag() - o2->imag() < 0.000000000001));
    o2++;
  }
