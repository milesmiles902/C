/*
 * @file .cpp
 * @brief GCP using [extended Euclid's algorith]
 *
 * Finding coefficients of a and b ie x and y in Bezout's identity
 * (a, b) = a*x+b*y
 * This is also used in finding Modular multiplicative inverse of a number
 * (A*B)%M == 1. Here is the MMI of A for given M, so extendedEuclid (A, M) gives B
 */
...
/*
 * @brief Implementation using iterative algorithm from Wikipedia
 * @param[in] A unsigned
 * @param[in] B unsigned
