...
struct Gammadev : Normaldev {
  ...
  Doub dev(){
    ...
    do{
      ...
      v=v*v*v;
      u=doub();
    } while (u > 1.-0.331*SQR(SQR(x)) && log(u) > 0.5*SQR(x) + a1*(1.-v+log(v)));
    ...
    if (lambda <5.){
      ...
      do{
        ...
        for (;;) {
          ...
          if (k<1024){
            if(logfact[k]<0.) logfac[k] = gammln(k+1.);
            lfac=logfact[k];
...
struct Binomialdev : Ran {
  ...
  Int dev() {
    ...
    if (swch == 0) {
      unfin = uo;
      for(j=0;j<5;j++){
        diff = unfin & (int64()^(pbits[j]? up : uz));
  ...
  VecDoub &dev() {
    ...
    for(i=0;i<mm;i++){
      do{
        ...
        y=abs(v)+0.386595;
        q=SQR(x)+y*(0.19600*y-0.25472*x);
      } while (q>0.26597 && (q>0.27846 || SQR(v) > -4.*log(u)*SQR(u)));
      spt[i]=v/u;
    }
    chol.elmult(spt,pt);
