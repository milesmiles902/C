...
struct Load1{
  ...
  Doub gmma,c2,dx;
  VecDoub y;
  Load1(Int nn,Int mm,Doub gmmaa,Doub cc2,Doub dxx) : n(nn), m(mm), gmma(gmmaa), c2(cc2), dx(dxx),y(3){}
...
Int main_sphfpt(void){
  ...
  for(j=0;j<MM;j++){
    Doub gmma=1.0;
    Doub q1=n;
    for(Int i=1;i<=m;i++) gmma*=-0.5*(n+1)*(q1--/i);
    ...
    Rhsfpt d(m,c2[j]);
    Score score;
    Shootf<Load1,Load2,Rhsfpt,Score> shootf(nvar,n2,x1,x2,xf,load1,load2,d,score);
    newt(v,check,shootf);
    if(check){
