struct LUdcmp {
  ...
  Doub d;                                   //Used by det
  LUdcmp(MatDoub_I &a=);                    //Constructor
  void solve(VecDoub_I &b, VecDoub_O &x);   //Solve for a single right-hand side
  void solve(MatDoub_I &b, MatDoub_O &x);   //Solve for multiple right-handed sides
  void inverse(MatDoub_O, &ainv);           //Calculate matrix inverse
  Doub det();                               //Return determinant of A
  void mprove(VecDoub_I &b, VecDoub_IO &x); //Discussed in Chapter 2.5
...
void LUcmp::solve(VecDoub_I &b, VecDoub_O &x) {
  Int i,ii=0,ip,j;
  Doub sum;
...
void LUdcmp::solve(MatDoub_I &b, MatDoub+O &x) {
  int i,j,m=b.ncols();
