...
HMM:HMM(MatDoub_I &aa, MatDoub_I &bb, VecInt_I &obss) :
  a(aa),b(bb),obs(obss),fbdone(0),mstat(a.nrows()),nobs(obs.size()),ksym(b.ncols()),alpha(nobs,mstat),beta(nobs,mstat),pstate(nobs,mstat),arnrm(nobs),brnrm(nobs)BIG(1.e20),BIGI(1./BIG {
  Int i,j,k;
  Doub sum;

  if(a.ncols() !=mstat) throw("transittion matrix not square");
  ...
  for(i=0;i<mstat;i++){
    ...
    if(abs(sum-1.)>0.01) throw("transititon amtrix not normalized");
    for(j=0;j<mstat;j++) a[i][j]/=sum;
  }
  for(i=0;i<mstat;i++){
    sum=0.;
    for(k=0;k<ksum;k++) sum+=b[i][k]; 
    if(abs(sum-1.)>0.01) throw("symbol prob matrix not nortmalized");
    for(k=0;k<ksym;k++) b[i][k]/=sum;
  ]
}

void HMM::forwadbackward(){
  Int i,j,t;
  Doub sum,asum.bsum;
  ...
  for(t=0;t<nobs;t++){
    ...
    for(i=0;i<msta;i++) pstate[t][i]/=sum;
  }
  fbdone=1;
}
