struct Fitmrq{
  ...
  void funks(const Doub, VecDoub_I &, Doub &, VecDoub_O &), const Doub TOL=1.e-3) : ndat(xx.size()),
  ma(aa.size()),x(xx),y(yy),sig(ssig),tol(TOL),funcs(funks),ia(ma),alpha(ma,ma),a(aa),covar(ma,ma){
    for(Int i=0;i<ma;i++) ia[i]=true;
  }
  
  void hold(const Int i, const Doub val) {ia[i]=false; a[i]=val;};
  void free(const Int {ia[i]=true;}
  void fit(){
    Int j,k,l,iter,done=0;
    Doub alamda=0.001,ochisq;
    VecDoub atry(ma),beta(ma),da(ma);
    mfit=0;
    for(j=0;j<ma;j++) if(ia[j]_ mfit++;
    ...
    for(iter=0;iter<ITMAX;iter++){
      ...
      for(j=0;j<mfit;j++){
        ...
        covar[j][i]=alpha[j][j]*(1.0+alamda);
