stuct Difeq{
  ...
  const VecDoub &x;
  Difeq(cont Int &mmm, const Int &nn, const Int &mptt, const Doub &hh,
    const Doub &cc2, const Doub &anormm, VecDoub_I &xx) : mm(mmm),
    n(nn),mpt(mptt),h(hh),c2(cc2),anorm(anormm),x(xx){}
  void smatrix(const Int k,const Int k2, const Int k2, const Int jsf,
    ...
    MatDoub_I &y){
      ...
      if(k==k1){
        ...
        s[0][jsf]=y[1][mpt-1]-(y[2][mpt-1]-c2)*y[0][mpt-1]/(2.0*(mm+1.0));
        s[1][3+indexv[0]]=1.0;
        s[1][3+indexv[1]]=0.0;
        s[1][3+indexv[2]]=0.0;
        s[1][jsf]=y[0][mpt-1]-anorm;
          ...
          s[1][jsf]=y[1][k]-y[1][k-1]-temp*((x[k]+x[k-1])*0.5*(mm+1.0)*(y[1][k]+y[1][k-1])-temp2*0.5*(y[0][k]+y[0][k-1]));
          s[2][jsf]=y[2][k2]-y[2][k-1];
      }
  }
}
