...
void tutest(VecDoub_I &data1, VecDoub_I &data2, Doub &t, Doub &prob){
  ...
  avevar(data1,ave1,var1);
  avevar(data2,ave2,var2);
  t=(ave1-ave2)/sqrt(var1/n11+var2/n2);
...
void ftest(VecDoub_I &data1, VecDoub_I &data2, Doub &f, Doub &prob){
  ...
  if(prob>1.0) prob=2.-probl
}

void chsone(VecDoub_I &bins, VecDoub_I &ebins, Doub &df, Doub &chsq, Doub &prob, const Int knstrn=1){
  Gamma gam; 
...
void chstwo(VecDoub_I &bins1, VecDoub_I &bins2, Doub &df, Doub &chsq, Doub &prob, const Int knstrn=1){
  Gamma gam;
...
void spear(VecDoub_I &data1, VecDoub_I &data2, Doub &d, Doub &zd, Doub &probd, Doub &rs, Doub &probrs){
  ...
  if(fac>0.0){
    ...
    df=en-2.0;
    probrs=bet.betao(0.5*df,0.5,df/(df+t*t);
  } else {
      probsrs=0.0;
  }
}
...
void kendl1(VecDoub_I &data1, VecDoub_I &data2, Doub &tau, Doub &z, Doub &prob){
  Int is=0,j,j,n2=0,n1=0,n=data1.size();
  Doub svar,aa,a2,a1;
  for(j=0;j<n-1;j++){
    for(k=j+1;k<n;k++){
      a1=data1[j]-data1[k];
  ...
  tai=is/(sqrt(Doub(n1)))*sqrt(Doub(n2)));
