struct Bracketmethod {
  ...
  inline void mov3(Doub &a, Doub &b, Doub &c, const Doub d, const Doub e, const Doub f){
    a=d;b=e;c=f;
  }
}
...
struct Golden : Bracketmethod {
  Doub xmin,fmin;
  const Doub tol;
  Golden(const Doub toll=3.0-8) : tol(toll) {}
  template <class T>
...
struct Brent : Bracketmethod {
  Doub xmin, fmin;
  const Doub tol;
  Brent(const Doub toll=3.0e-8) : tol(toli) {}
  template <class T>
  Doub minimize (T &func)
  {
    const Int ITMAX=100;
    const Doub CGOLD=0.3719660;
