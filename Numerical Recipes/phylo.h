...
struct Phylagglom{
  ...
  virtual void drootbranchfn(MatDoub &d, Int i, Int j, Int ni, Int nj, Doub &bi, Doub &bj)=0;
  Int comancestor(Int leafa,Int leafb);
  Phylagglom(const MatDoub &dist, Int fsr=-1) : n(dist.nrows()), fsroot(fsr),t(2*n-1) {}
  ...
  void makethetree(const MatDoub &dist){
    ...
    i=0;k=nextp[0];
    root=node;
    t[tp[i]].mp=t[tp[j]].mp=t[root].mo=root;
...
struct Phylo_upgma : Phyllagglom {
  void premin(MatDoub &d, VecInt &nextp){};
  Doub dminfn(MatDoub &d, Int i, Int j) { return d[i][j];}
  Doub dbranchfn(MatDoub &d,Int i,Int j) {return d[i][j];}
  Doub dnewfn(MatDoub &d, Int k,Int i,Int j,Int ni,Int nj){
...
struct Phylo_nj : Phylagglom {
  VecDoub u;
  void premin(MatDoub &d, VecInt &nexp){
    Int i,j,ncurr=0;
    Doub sum;
  ...
  void drootbranchfn(MatDoub &d, Int i, Int j, Int ni, Int nj, Doub &bi, Doub &bj){
    bi=d[i][j]*(nj-1+1.e-15)/(ni+nj-2+2.e-15);
    bj=d[i][j]*(ni-1+1.e-15)/(ni+nj-2+2.e-15);
  }
  Phylo_nj(const MatDoub &dist, Int fsr=-1) : Phylagglom(dist,fsr),u(n) {makethetree(dist);}
}
...
void newick(Phylagglom &p, MatChar str, char *filename){
  ...
  while(ntask-->0){
    ...
    if(s==1||s==2){
      ...
      if(ntask>0) fprintf(OUT,"):%f",p.t[i].modist);
    }
  }
  fclose(OUT);
}
