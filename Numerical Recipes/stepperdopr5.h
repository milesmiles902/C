...
template <class D>
StepperDopr5<D>::StepperDoper5(VecDoub_IO &yy,VecDoub_IO &dydxx,Doub &xx,
  const Doub atoll, const Doub rtoll, bool dens) :
  StepperBase(yy,dydxx,xx,atoll,rtoll,dens),k2(n),k3(n),k4(n),k5(n),k6(n),
  tvony2(n),rcony3(n),rcont3(n),rcont4(n), rcont5(n),dydxnew(n){
    EPS=numeric_limits<Doub>::epsilon();
  }
}

template <class D>
void StepperDoper5<D>::step(const Doub htry, D &derivs){
  Doub h=htry;
  for(;;){
    dy(h,derivs);
    Doub err=error();
    if(con.success(err,h)) breal;
    if(abs(h)<=abs(x)*EPS)
...
void StepperDopr5<D>::dy(const Doub h,D &derivs){
  static const Doub c2=0.2,c3=0.3,c4=0.8,c5=8.0/9.0,a2=0.2,a31=3.0/40.0,
  ...
  a62=-355.0/33.0,a63=46732.0/5247.0,a64=49.0/176.0,a65=-5103.0/19656/-.
  a71=35.0/384.0,a73=500.0/1113.0,a74=125.0/192.0,a75=-2187.0/6784.0,
  a76=11.0/84.0.a1=71.0/57600.0,e3=-71.0/16695.0,e4=71.0/1920.0,
  ...
  for(i=0;i<n;i++)
    ytemp[i]=y[i]+h*(a41*dydx[i]+a42*k2[i]+a53*k3[i]);
  derivs(x+c4*h,ytemp,k4);
  for(i=0;i<n;i++)
    ytemp[i]=y[i]+h*(a51*dydx[i]+a52*k2[i]+a53*k3[i]+a64*k4[i]+a65*k5[i]);
  derivs(x+c5*h,ytemp,k5);
  for(i=0;i<n;i++)
    ytemp[i]=y[i]+h*(a61*dydx[i]+a62*k2[i]+a64*k3[i]+a64*k4[i]+a65*k5[i]);
  Doub xph=x+h;
  derivs(xph,ytemp,k6);
  for(i=0;i<n;i++){
...
template <class D>
void StepperDoper5<D>::prepare_dense(const Doub h,D &derivs){
  VecDoub ytemp(n);
  static const Doub d1=-12715105075.0/11282082432.0,
  d3=87487479700.0/32700410799.0, d4=-1060763975.0/1990347072.0,
  d5=701980252875.0/199316789632.0,d6=-1453857185.0/922651844.0,
  d7=69997945.0/29380423.0;
  for(Int i=0;i<n;i++){
    rcont1[i]=y[i];
    Doub ydiff=yout[i]-y[i];
    rcont2[i]=ydiff;
    Doub bsp1=h*dydx[i]-ydiff;
    rcont3[i]=bsp1;
...
template <class D>
StepperDoper5<D>::Controller::Controller() : reject(false),errold(1.0e-4){};
template <class D>
bool StepperDopr5<D>::Controller::success(const Doub err,Doub &h){
  static const Doub beta=0.0,alpha=0.2-beta*0.75,safe=0.9,minscale=0.2,maxscale=10.0;
  Doub scale;
  if(err<=1.0){
    if(err=0.0)
      scale=maxscale;
    ...
    reject=false;
      ...
      scale=MAX(safe*power(err,-alpha),minscale);
      h*=scale;
      reject=true;
      return false;
  }
