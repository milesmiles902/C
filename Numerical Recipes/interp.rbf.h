...
struct RBF_interp{
  ...
  : dim(ptss.ncols()), n(ptss.nrows()), pts(ptss), vals(valss), w(n), fn(func), norm(nrbf){
    Int i,j;
    Doub sum;
    MatDoub rbf(n,n);
    VecDoub rhs(n);
...
struct RBF_multiquadratic : RBF_fn {
  Doub r02;
  RBF_multiquadratic(Doub scale=1.) : r02(SQR(scale)) {}
...
struct RBF_inversemultiquadric : RBF_fn {
  Doub r02;
  RBF_inversemultiquadri(Doub scale = 1.) : r02(SQR(scale)) {}
...
struct Shep_interp{
  ...
  : dim(ptss.ncols()), n(ptss.nrows()), pts(ptss), vals(valss), pneg(-p) {}
  ...
  Doub rad(const Doub *pi, const Doub *p2){
    Doub sum=0;
    for (Int i=0;i<dim;i++) sum += SQR(p1[i]-p2[i]);
    return sqrt(sum);
  }
