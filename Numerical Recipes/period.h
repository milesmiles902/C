void period(VecDoub_I &x, VecDoub_I &y, const Doub ofac, const Doub hifac, VecDoub_O &px, VecDoub_O &py,
  Int &nout, Int &jmax, Doub &prob){
  ...
  for(i=0;i<nout;i++){
    ...
    for(j=0;j<n;j++){
      ...
      sumsy+=yy*ss;
      sumcy+=yy*cc;
      wr[j]=((wtemp=wr[j])*wpr[j]-wo[j]*wpi[j])+wr[j];
