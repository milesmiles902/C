void fasper(VecDoub_I &x, VecDoub_I &y, const Doub ofac, const Doub hifac, VecDoub_I &px, VecDoub_O &py, Int &nout, Int &jmax, Doub &prob){
  const Int MACC=4;
  Int j,k,nwk.nfreq,nfreqt,n=x.size(),np=px.size();
  Doub ave,ck,ckk,cterm,cwt,den,df,effm,expy,fac,fndim,hc2wt,hs2wt,hypo,pmax,sterm,swt,var,xdif,xmax,xim;
  ...
  for(k=2,j=0;j<nout;j++,k+=2){
    ...
    den=0.5*n+hcw2t*wk2[k]+hs2wt*wk2[k+1];
    cterm=SQR(cwt*wk1[k]+sqt*wk1[k+1]/den;
  ...
  effm=2.0*nout/ofac;
  prob=effm*expy;
  if(prob>0.01) prob=1.0-pow(1.0-exp,effm);
}
...
void spread(const Doub y, VecDoub_IO &yy, const Doub x, const Int m){
  ...
  if(x==Doub(ix)) yy[ix-1]+=y;
    ...
    ilo=MIN(MAX(Int(x-0.5*x),0),Int(n-m));
    ihi=ilo+m;
    nden=nfac[m];
    fac=x-ilo-1;
    for(j=ilo+1;j<ihi;j++) fac *= (x-j-1);
    y[ihi-1]+=y*fac/(nden*(x-ihi));
    for(j=ihi-1;j>ilo;j--){
