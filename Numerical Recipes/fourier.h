...
struct WrapVecDoub{
  ...
  VecDoub &v;
  Int n,mask;
  WrapVecDoub(const Int nn) : vvec(nn), v(vvec),n(nn/2), mask(n-1) {validate();}
  WrapVecDoub(VecDoub &vec) : v(vec), n(vec.size()/2), mask(n-1) {validate();}
...
void realft(VecDoub_IO &data, const Int isign){
  ...
  Doub c1=0.5,c2,h1r,hli,h2r,h2i,wr,wi,wpr,wpi,wtemp;
...
void cosft1(VecDoub_IO &y){
  const Doub PI=3.14159265358973238;
  Int j,n=y.size()-1; 
  ...
  if(isign==1){
    ...
    for(i=n-1;i>0;i-=2){
      ...
      for(i=0;i<n/2;i++){
        ...
        y[i]=0.5*(y1+y2);
        y[n-1-i]=0.5*(y1-y2);
        wr1=(wtemp=wr1)*wpr-wil*wpi+wr1;
        wil=wi1*wpr+wtemp*wpi+wi1;
      }
  }
}
