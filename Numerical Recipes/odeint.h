...
struct Odeint {
  ...
  VecDoub &ystart;
  Output &out;
  typename Stepper::Dytype *derivs;
  Stepper s;
  Int nstp;
  Doub x,h;
  Odeint(VecDoub_IO &ystartt,const Doub xx1,const Doub xx2,
    const Doub atol, const Doub rtol, const Doub h1,
    const Doub hminn,Output &outt,typename Stepper::Dtype &derivs);
  void integrate();
}

template<class Stepper>
Odeint<Stepper>::Odeint(VecDoub_IO &ystartt,const Doub xx1, const Doub x2,
   ...
   x1(xx1),x2(xx2),hmin(hminn),dense(outt.dense),out(outt),derivs(derives),
...
struct StepperBase {
  ...
  Int n,neqn;
  VecDoub yout,yerr;
  StepperBase(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx, const Doub atoll,rtol(rtoll),dense(dens),n(y.size()),neqn(n),yout(n),yerr(n){};
}
...
struct Output {
  ...
  Output(const Int nsaveee) : kmax(500),nsave(nsavee),count(0),xsave(kmax){
