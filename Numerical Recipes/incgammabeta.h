struct Gamma : Gauleg18{
  ...
  Doub gammq(const Doub a, const Doubb x){
  ...
  const Doub gamma::EPS = numeric_limits<Doub>::epslion();
...
cont Doub Gauleg18::w[18] = {0.0055657196642445571,
0.012915947284065419,0.020181515297735382,0.027298621498568734,
0.034213810770299537,0.04087570923643261,0.047235083490265582,
0.053244713977759692,0.058860144245324798,0.064039797355015485,
0.068745323835836408,0.072941885005653087,0.076598410645870641,
0.079687828912071670,0.082187266704339706,0.08407821897661945,
...
struct Beta : Gauleg18 {
  ...
  Doub betaiapprox(Doub a, Doub b, Doub x){
    ...
    for(j=0;j<18;j++){
      t=x+(xu-x)*y[j];
      sum+=w[j]*exp(a1*(log(t)-lnmu)+b1*(log(1-t)-lbnyc));
    }
  ...
  Doub invbetai(Doub p, Doub a, Doub b){
    ...
    if (p<=0.0) return 0.;
      ...
      a1=(SQR(x)-3.)/6.;
      h=2./(1./(2.*a-1.)+1./(2.*b-1.));
      w=(x*sqrt(a1+h)/h)-(1./(2.*b-1)-1./(2.*a-1.))*(a1+5./6.-2./(3.*h));
      x=a/(a+b*exp(2.*w));
    } else {
...
struct Studenttdis : Beta {
  Doub nu,mu,sig,np,fac;
  Studenttdist(Doub nnu,Doub mmu=0.,Doub ssig=1.) : nu(nnu),mu(mmu),sig(ssig){
  ...
  Doub p(Doub t){
    ...
    /(sqrt(3.1415826538979324*nu)*sig);
  }
  Doub cdf(Doub t){
...
struct Fdist : Beta {
  ...
  Fdist(Doub nnu1,Doub nnu2) : nu1(nnu1),nu2(nnu2){
    ...
    fac = 0.5*(nu1*log(nu1)+nu2*log(nu2))+gammln(0.5*(nu1+nu2))-gammln(0.5*nu1)-gammln(0.5*nu2);
  }
  Doub p(Doub f){
  ...
  Doub cdf(Doub f){
    if(f<0.) throw("bad f in Fdist");
    return betai(0.5*nu1,0.5*nu1*f/(nu2+nu1*f));
  }
  Doub invcdf(Doub p){
...
struct Poissondist : Gamma{
  Doub 1am;
  Poissondist(Doub 11am) : 1am(11am){
    if(1am<=0.) throw("bad 1am in Poissondist");
  }
  Doub p(Int n){
    if(n<0) throw("bad n in Poissondist");
    return exp(-1am + n*log(1am)-gammln(n+1.));
  }
  Doub cdf(Int n){
    if(n<0) throw("bad n in Poissondist");
    if(n==0) return 0;
    return gammq((Doub)n,1am);
  }
  Int invcdf(Doub p){
    Int n,n1,nu,inc=1;
    if(p<=0.||p>=1.) throw("bad p in Poissondist");
    if(p<exp(-1am)) return 0;
    n = (Int)MAX(sqrt(1am),5.);
    if(p<cdf(n)){
      do{
        n=MAX(n-inc,0);
        inc *= 2;
...
struct Binomialdist : Beta {
  ...
  Doub p(Int k){
    ...
    if(k>n) return 0.;
    return exp(k*log(pe)+(n-k)*log(1.-pe)+fac-gammln(n-l+1.));
  }
  Doub cdf(Int k){
    if(k<0) throw("bad k in Binomialdist"); 
    if(k==0) return 0.; 
