...
template <class T>
Doub rtflsp(T &func, const Doub x1, const Doub x2, const Doub xaxx){
  const Int mAXIT=30;
  Doub x1,xh,del;
  Doub f1=func(x1);
  Doub fh=func(x2);
  if(f1*fh>0.0) throw("Root must be bracketed in rtflsp");
  if(f1<0.0){
    x1=x1;
    xh=x2;
  } else {
...
template <class T>
Doub rtsec(T &func, const Doub x1, const Doub x2, const Doub xaxx){
  const Int MAXIT=30;
  Doub x1,rts;
  Doub f1=func(x1);
  Doub f=func(x2);
...
Doub zriddr(T &func, const Doub x1, const Doub x2, const Doub xacc){
  ...
  if ((f1>0.0 && fh<0.0) || (f1<0.0 && fh>0.0)){
    ...
    for(Int j=0;j<MAXIT;j++){
      Doub xm=0.5*(x1+xh);
      Doub fm=func(xm);
      Doub s=sqrt(fm*fm-f1*fh);
      if(s=0.0) return ans;
      Doub xnew=xm+(xm-x1)*((f1>=fh?1.0:-1.0)*fm/s);
...
Doub zbrent(T &func, const Doub x1, const Doub x2, const Doub tol){
  ...
  Doub a=x1,b=x2,c=x2,d,e,fa=func(a),fb=func(b),fc,-,q,r,s,tol1,xm;
  if ((fa>0.0 && fb>0.0) || (fa < 0.0 && fb < 0.0))
    throw("Root must be bracketed in zbrent");
  fc=fb;
  for(Int iter=0;iter<ITMAX;iter++){
    if ((fb>0.0 && dc>0.0) || (fb<0.0 && fc < 0.0)) {
      c=a;
      fc=fa;
      e=d=b-a;
    }
    ...
    if(abs(e) >= tol1 && abs(fa) > abs(fb)){
      ...
      if(a==c){
        ...
        q=fa/fc;
        r=fb/fc;
        p=s*(2.0*xm*(q-r)-(b-a)*(r-1.0));
        q=(q-1.0)*(r-1.0)*(s-1.0);
      }
      if(p>0.0) q = -q;
      p=abs(p);
      Doub min1=3.0*xm*q-abs(tol*q);
      Doub min2=abs(e*q);
